...
namespace estate_executor {
    ...
    int assemble_account_number(int secret_modifier){
        int secret =0;
        secret += zhang:: bank_number_part( secret_modifier);
        secret += khan:: bank_number_part( secret_modifier);
        secret += garcia:: bank_number_part( secret_modifier);
        return secret;
    }

    int assemble_code(){
        int sumBlue= zhang::blue:: code_fragment() + garcia::blue:: code_fragment() +  khan::blue:: code_fragment();
        int sumRed= garcia::red:: code_fragment() + khan::red:: code_fragment() + zhang::red:: code_fragment();
